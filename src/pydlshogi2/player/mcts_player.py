...
class MCTSPlayer(BasePlayer):
    ...
    def usi(self):
        ...
        print(
            "option name gpu_id type spin default "
            + str(DEFAULT_GPU_ID)
            + " min -1 max 7"
        )
        print(
            "option name batchsize type spin default "
        ...
        print(
            "option name c_puct type spin default "
            + str(int(DEFAULT_C_PUCT * 100))
            + " min 10 max 1000"
        )
        print(
        ...
        print(
            "option name time_margin type spin default "
            + str(DEFAULT_TIME_MARGIN)
            + " min 0 max 1000"
        )
        print(
            "option name byoyomi_margin type spin default "
            + str(DEFAULT_BYOYOMI_MARGIN)
            + " min 0 max 1000"
        )
        print(
            "option name pv_interval type spin default "
            + str(DEFAULT_PV_INTERVAL)
            + " min 0 max 10000"
        )
    ...
    def set_limits(
        ...
        if infinite or ponder:
            ...
            if self.remaining_time is None and byoyomi is None and inc is None:
                ...
                self.minimum_time = 0
                self.remaining_time = (
                    int(self.remaining_time) if self.remaining_time else 0
                )
                inc = int(inc) if inc else 0
                self.time_limit = (
                    self.remaining_time
                    / (14 + max(0, 30 - self.root_board.move_number))
                    + inc
                )
                # 秒読みの場合
                if byoyomi:
    ...
    def go(self):
        ...
        if current_node.value == VALUE_WIN:
            ...
            if matemove != 0:
                print(
                    "info score mate 3 pv {}".format(move_to_usi(matemove)), flush=True
                )
        ...
        if not self.root_board.is_check():
            ...
            if matemove:
                print(
                    "info score mate 1 pv {}".format(move_to_usi(matemove)), flush=True
                )
                return move_to_usi(matemove), None
        ...
        if self.halt is None and len(current_node.child_move) == 1:
            if current_node.child_move_count[0] > 0:
                ...
                return (
                    ...
                    move_to_usi(ponder_move) if ponder_move else None,
                )
            else:
            ...
            for i in range(len(current_node.child_move)):
                print(
                    "{:3}:{:5} move_count:{:4} nn_rate:{:.5f} win_rate:{:.5f}".format(
                        ...
                        current_node.child_sum_value[i]
                        / current_node.child_move_count[i]
    ...
    def check_interruption(self):
        ...
        if (
            ...
            and (
                first < second * 1.5
                or current_node.child_sum_value[first_index]
                / child_move_count[first_index]
                < current_node.child_sum_value[second_index]
                / child_move_count[second_index]
            )
        ):
            # 探索時間を2倍に延長
            self.time_limit *= 2
